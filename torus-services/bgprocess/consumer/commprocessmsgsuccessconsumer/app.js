...
@Last Error Code : 'ERR_COMM_MSG_SUCCESS_APP_0001'
*/
...
try {
    ...
    var servicePath = 'CommProcessMsgSuccessConsumer';
    ...
    if (!reqCluster.isMaster) {
        ...
        process.title = 'Torus_Bg_CommProcessMsgSuccessConsumer';
        ...
        function AfterInitDBListener() {
            var reqConsumer = require('./routes/Consumer');
            var topic = 'FX_COMM_PROCESS_MSG_SUCCESS'; // For Postgrez DB
            var group = 'FX_COMM_PROCESS_MSG_SUCCESS_GRP2';
            var headers = {
            ...
            reqInstanceHelper.GetTenantTimezoneInfo(GetTenantTimezoneInfoReqObj, function (GetTenantTimezoneInfoStatus) {
                if (GetTenantTimezoneInfoStatus) {
                    ...
                    reqInstanceHelper.GetAllUniqueTranDBKeys(GetAllUniqueTranDBKeysObj, function (error, arrUniqueTranDBKeys, isTenantMultiThreaded) {
                        if (error) {
                            reqInstanceHelper.PrintError(servicePath, objLogInfo, 'ERR_COMM_MSG_SUCCESS_APP_0001', 'Error in GetAllUniqueTranDBKeys()...', error);
                        } else if (isTenantMultiThreaded && !arrUniqueTranDBKeys.length) {
                            reqInstanceHelper.PrintError(servicePath, objLogInfo, 'ERR_COMM_MSG_SUCCESS_APP_0002', 'Tenant Multi Thread is Enabled in Redis Service Param. But There is No TRANDB keys, so Workers will not be Created...', '');
                        } else {
                    ...
                    reqInstanceHelper.PrintError(servicePath, objLogInfo, 'ERR_COMM_MSG_SUCCESS_APP_0003', 'Error While Getting Timezone Informations based on Tenant', '');
                    return;
                }
            });
    ...
    reqInstanceHelper.PrintError(servicePath, objLogInfo, 'ERR_COMM_MSG_SUCCESS_APP_0000', 'Catch Error while starting COMM_PROCESS_MSG_SUCCESS_CONSUMER Service...', error);
}