            ...
            if (pError) {
                ...
                if (pResult.rows.length > 0) {
                    ...
                    for (var i = 0; i < pResult.rows.length; i++) {
                        _ParseDTTInfo(pResult.rows[i], pResult.rows[i]['app_id'], pResult.rows[i]['relation_json'], htTemp, htTempTarget);
                    }
    ...
    */
        ...
        reqFXDBInstance.GetTableFromFXDB(dep_cas_instance, 'DTT_INFO', ['DTT_DFD_JSON', 'DTT_CODE', 'NEED_AUDIT_LOG', 'NEED_SOLR_INDEX'], {
            ...
            if (pError) {
                ...
                var htTempDTT = htDTTInfo.get(pDttInfoReqObj.headers['routingkey']);
                for (var v = 0; v < pResult.rows.length; v++) {
                    const element = pResult.rows[v];
                    var relationJson = htTempDTT.get(element.dtt_code);
                    if (relationJson) {
                        element.dtt_dfd_json = (element.dtt_dfd_json).replace(/\\/g, '');
                        relationJson.RELATION.DTT_DFD_JSON = (element.dtt_dfd_json && JSON.parse(element.dtt_dfd_json));
                        htTempDTT.put(element.dtt_code, relationJson);
                    }
...
function _ParseDTTInfo(DTInfoObj, pAppID, pDTRelation, pHashTable, htTempTarget) {
    try {
        var objRelationJson = JSON.parse(pDTRelation) || [];
        if (objRelationJson.length) {
            objRelationJson[0].DT_CODE = DTInfoObj.dt_code;
            objRelationJson[0].DT_DESCRIPTION = DTInfoObj.dt_description;
        }
        var tmpStr = ''
...
function _GetHierarchyDTT(pAppID, pRelationJson, pHashTable, htTempTarget) {
    try {
        ...
        for (var i = 0; i < objRelationJson.CHILD_DTT_RELEATIONS.length; i++) {
            objRelationJson.CHILD_DTT_RELEATIONS[0].DT_CODE = objRelationJson.DT_CODE;
            objRelationJson.CHILD_DTT_RELEATIONS[0].DT_DESCRIPTION = objRelationJson.DT_DESCRIPTION;
