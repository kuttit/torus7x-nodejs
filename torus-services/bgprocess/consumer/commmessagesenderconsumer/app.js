/*
 Descriptions - Node app.js file to start Comm Message Sender Consumer service
@Last Error Code : 'ERR_COMM_MSG_SENDER_APP_0001'
*/
...
try {
    ...
    var servicePath = 'commmessagesender';
    ...
    if (!reqCluster.isMaster) {
        ...
        process.title = 'Torus_Bg_CommMessageSenderConsumer';
        ...
        function AfterInitDBListener() {
            var reqConsumer = require('./routes/Consumer');
            var topic = 'FX_COMM_PROCESS_MSG'; // For Postgrez DB
            var group = 'FX_COMM_PROCESS_MSG_GRP';
            var headers = {
            ...
            reqInstanceHelper.GetTenantTimezoneInfo(GetTenantTimezoneInfoReqObj, function (GetTenantTimezoneInfoStatus) {
                if (GetTenantTimezoneInfoStatus) {
                    ...
                    reqInstanceHelper.GetAllUniqueTranDBKeys(GetAllUniqueTranDBKeysObj, function (error, arrUniqueTranDBKeys, isTenantMultiThreaded) {
                        if (error) {
                            reqInstanceHelper.PrintError(servicePath, objLogInfo, 'ERR_COMM_MSG_SENDER_APP_0001', 'Error in GetAllUniqueTranDBKeys()...', error);
                        } else if (isTenantMultiThreaded && !arrUniqueTranDBKeys.length) {
                            reqInstanceHelper.PrintError(servicePath, objLogInfo, 'ERR_COMM_MSG_SENDER_APP_0002', 'Tenant Multi Thread is Enabled in Redis Service Param. But There is No TRANDB keys, so Workers will not be Created...', '');
                        } else {
                    ...
                    reqInstanceHelper.PrintError(servicePath, objLogInfo, 'ERR_COMM_MSG_SENDER_APP_0003', 'Error While Getting Timezone Informations based on Tenant', '');
                    return;
                }
            });
    ...
    reqInstanceHelper.PrintError(servicePath, objLogInfo, 'ERR_COMM_MSG_SENDER_APP_0000', 'Catch Error while starting Attachment Consumer Service...', error);
}