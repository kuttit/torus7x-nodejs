...
router.post('/DeleteItem', function (appRequest, appResponse) {
    try {
        ...
        reqLogInfo.AssignLogInfoDetail(appRequest, function callbackAssignLogInfoDetail(pLogInfo, pSessionInfo) {
            ...
            var mOrm = 'knex';
            var objRelation = '';
            var ClearCache = strInputParam.CLEAR_CACHE;
            try {
                _PrintInfo("Begin");
                ...
                _InitializeTrnDB(pHeaders, function callbackInitializeDB(pStatus) {
                    reqAuditLog.GetProcessToken(mTranDB, objLogInfo, function (err, prct_id) {
                        try {
                            if (err) {
                                _PrintError('ERR-CODE', 'Error on GetProcessToken()', err);
                                return _SendResponse(pStatus, 'ERR-CODE', 'Error on GetProcessToken() ', err, null);
                            }
                            strTokenId = prct_id;
                            // Initialize params
                            ...
                            _PrintError('ERR-CODE', 'Catch Error on GetProcessToken()', error);
                            return _SendResponse(pStatus, 'ERR-CODE', 'Catch Error on GetProcessToken() ', err, null);
                        }
                    });
                });
            } catch (error) {
                            ...
                            try {
                                if (pResult.rows.length > 0) {
                                    ...
                                    function deleteIt(pItem) {
                                        ...
                                        var strDTCode = pItem.DT_CODE;
                                        var strDTTCode = pItem.DTT_CODE;
                                        ...
                                        if (strTemp.Status == 'SUCCESS') {
                                            ...
                                            if (strDTTCategory) {
                                                ...
                                                _SendResponse('FAILURE', 'ERR-HAN-40341', 'DTT Category Not found ', '', "DTT Category Master/Transaction not available in DT_INF's relation json");
                                            }
            ...
            function clearRedisCache(clearMode) {
                try {
                    reqCacheRedisInstance.GetRedisConnection(pHeaders, function (pRedisClient) {
                        if (pRedisClient) {
                            reqInsHelper.PrintInfo('DELETE_TRAN', 'Cache type | ' + clearMode, objLogInfo);
                            if (clearMode == 'ALL') {
                                doClearCache();
                        ...
                        function doClearCache(db) {
                            reqCacheRedisInstance.ClearCache(pHeaders, {
                                ...
                                try {
                                    reqInsHelper.PrintInfo('DELETE_TRAN', 'Clear cache Result ' + result, objLogInfo);
                                } catch (error) {
                                    reqInsHelper.PrintInfo('DELETE_TRAN', 'Clear cache Exception occured ' + error, objLogInfo);
                                }
                            });
                        }
                    });
                        ...
                        if (pError)
                            ...
                            if (strLockedBy == null || strLockedBy == undefined || strLockedBy == strUId || strLockedBy == '0' || strLockedBy == '') {
                                if (DBType == 'pg' || DBType == 'mysql') {
                                    ...
                                    var strQuery = "SELECT TS.DT_CODE, TS.DTT_CODE, TS.TS_ID, TS.PARENT_TS_ID, TS.TRN_ID AS ITEM_ID, TS.LOCKED_BY from TRANSACTION_SET TS WHERE DT_CODE = '" + strDTCode + "'  START WITH trn_id in ( " + pItemId + " ) " + " CONNECT BY PRIOR TS_ID = PARENT_TS_ID ORDER SIBLINGS BY TS_ID";
                                }
                                            ...
                                            reqAsync.parallel({
                                                ...
                                                function (err, results) {
                                                    ...
                                                    if (results.DeleteTargetTable.Status == 'SUCCESS' && results.DeleteSolrIndex.Status == 'SUCCESS' && results.DeleteAttachment.Status == 'SUCCESS' && results.DeleteComment.Status == 'SUCCESS' && results.DeleteTransactionSet.Status == 'SUCCESS') {
                                                        if (ClearCache) {
                                                            reqInsHelper.PrintInfo('DELETE_TRAN', 'Cache type | ' + ClearCache, objLogInfo);
            ...
            function _DeleteTargetTable(pTargetTable, pKeyColumn, pRowsToDelete, pCallback) {
                try {
                    ...
                    var updateValue = `prct_id='${strTokenId}',MODIFIED_BY_NAME='${objLogInfo.LOGIN_NAME}',MODIFIED_DATE='${reqDateFormatter.GetTenantCurrentDateTime(pLogInfo.headers, pLogInfo)}' `
                        ...
                        DeleteParentTable: function (cbPar) {
                            ...
                            if (Item_Id != '') {
                                ...
                                async function delItem() {
                                    var arrstrQuery = [];
                                    var strqryWhere = " WHERE " + pKeyColumn + " IN (" + Item_Id + ")"
                                    var strUpdateQry = `update ${pTargetTable} set ${updateValue} ${strqryWhere} `
                                    var strQuery = "DELETE FROM " + pTargetTable + strqryWhere;
                                    arrstrQuery.push(strUpdateQry, strQuery);
                                    var TranJorneyQryWhere = " WHERE DTT_CODE='" + strParentDTT + "' AND TRN_ID IN (" + Item_Id + ")"
                                    var strTJUpdateQry = `update TRANSACTION_JOURNEY set prct_id='${strTokenId}' ${TranJorneyQryWhere}  `
                                    var TranJorneyQuery = "DELETE FROM TRANSACTION_JOURNEY " + TranJorneyQryWhere;
                                    arrstrQuery.push(strTJUpdateQry, TranJorneyQuery);
                                    var TranJorneyDtlQryWhere = "WHERE DTT_CODE='" + strParentDTT + "' AND TRN_ID IN (" + Item_Id + ")"
                                    var tjdUpdateQry = `update TRANSACTION_JOURNEY_DETAIL set prct_id='${strTokenId}' ${TranJorneyDtlQryWhere}`
                                    var TranJorneyDtlQuery = "DELETE FROM TRANSACTION_JOURNEY_DETAIL " + TranJorneyDtlQryWhere;

                                    arrstrQuery.push(tjdUpdateQry, TranJorneyDtlQuery);

                                    for (var i = 0; i < arrstrQuery.length; i++) {
                                        await executeQry(mTranDB, arrstrQuery[i], objLogInfo);
                                    }
                            ...
                            async function delchildItem() {
                                ...
                                if (strTemp.Status == 'SUCCESS') {
                                    ...
                                    if (arrItemID.length > 0) {
                                        var arrQries = []
                                        var strupdateQuery = "UPDATE " + strTargetTable + " SET " + updateValue + "WHERE " + strKeyColumn + " IN (" + arrItemID.join() + ")";
                                        var strQuery = "DELETE FROM " + strTargetTable + " WHERE " + strKeyColumn + " IN (" + arrItemID.join() + ")";
                                        var UpdateTjQry = "UPDATE TRANSACTION_JOURNEY SET " + updateValue + "WHERE DTT_CODE='" + DTT + "' AND TRN_ID IN (" + arrItemID.join() + ")";
                                        var TranJorneyQuery = "DELETE FROM TRANSACTION_JOURNEY  WHERE DTT_CODE='" + DTT + "' AND TRN_ID IN (" + arrItemID.join() + ")";
                                        var UpdateTJDtlQuery = "UPDATE TRANSACTION_JOURNEY_DETAIL SET " + updateValue + " WHERE DTT_CODE='" + DTT + "' AND TRN_ID IN (" + arrItemID.join() + ")";
                                        var TranJorneydtlQuery = "DELETE FROM TRANSACTION_JOURNEY_DETAIL  WHERE DTT_CODE='" + DTT + "' AND TRN_ID IN (" + arrItemID.join() + ")";
                                        arrQries.push(strupdateQuery, strQuery, UpdateTjQry, TranJorneyQuery, UpdateTJDtlQuery, TranJorneydtlQuery)
                                        for (var i = 0; i < arrQries.length; i++) {
                                            await executeQry(mTranDB, arrQries[i], objLogInfo);
                                        }
                                        callback()
                                    } else
                                        callback(null);
            ...
            function _DeleteTransactionSet(pRowsToDelete, pCallback) {
                try {
                    ...
                    if (Item_Id != '') {
                        delTs()
                        async function delTs() {
                            var updateTs = `UPDATE TRANSACTION_SET SET prct_id='${strTokenId}',MODIFIED_BY_NAME='${objLogInfo.LOGIN_NAME}',MODIFIED_DATE='${reqDateFormatter.GetTenantCurrentDateTime(pLogInfo.headers, pLogInfo)}' WHERE TS_ID IN ( ${Item_Id} )`
                            var strQuery = "DELETE FROM TRANSACTION_SET WHERE TS_ID IN (" + Item_Id + ")";
                            var arrTs = [];
                            arrTs.push(updateTs, strQuery);
                            for (var i = 0; i < arrTs.length; i++) {
                                await executeQry(mTranDB, arrTs[i], objLogInfo);
                            }
                            return _PrepareAndSendCallback('SUCCESS', '', '', null, null, pCallback);
                        }
                        ...
                        reqAsync.parallel({
                            DeleteTranIndex: function (parCB) {
                                ...
                                if (TSIDIndex.length > 0)
                                    reqSolrHelper.SolrDelete(pHeaders, 'dynamic_core', 'TS_ID', TSIDIndex, objLogInfo, function callbackDeleteTranIndex(pStatus) {
                                        parCB(null, pStatus);
                                    });
                                else
                                    parCB(null, 'SUCCESS');
                            ...
                            DeleteATMTIndex: function (parCB) {
                                ...
                                if (ATMTIndex.length > 0)
                                    reqSolrHelper.SolrDelete(pHeaders, 'static_core', 'filename', ATMTIndex, objLogInfo, function callbackDeleteATMTIndex(pStatus) {
                                        parCB(null, pStatus);
                                    });
                                else
                                    parCB(null, 'SUCCESS');
            ...
            function _InitializeParams(pClientParam, pSessionInfo, pCallback) {
                try {
                    ...
                    if (pClientParam['JSON_DATASET'] != undefined && pClientParam['JSON_DATASET'] != '')
                        strJsonDataSet = pClientParam['JSON_DATASET'].toString();
            ...
            function executeQry(pSession, query, objLogInfo) {
                return new Promise((resolve, reject) => {
                    try {
                        reqTranDBHelper.ExecuteSQLQuery(pSession, query, objLogInfo, function (pResult, pError) {
                            if (pError) {
                                reject(pError)
                            } else {
                                resolve()
