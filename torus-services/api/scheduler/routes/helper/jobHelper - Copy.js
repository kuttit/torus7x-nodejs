...
function createJob(mResCas, QUERY, objLogInfo, APP_ID, ROUTING_KEY, JOB_NAME, JOB_DESCRIPTION, TEMPLATE_NAME, JOB_TYPE, CALLING_METHOD, JOB_MODE, OBJECT_NAME, PARAM_JSON, SCHEDULER_INFO, STATUS, CREATED_BY, CREATED_DATE, JOB_RULE, callback) {
    try {
        var resdata = {};
...
// To create a new job
function createJobNew(mResCas, QUERY, objLogInfo, APP_ID, ROUTING_KEY, JOB_NAME, JOB_DESCRIPTION, TEMPLATE_NAME, JOB_TYPE, CALLING_METHOD, JOB_MODE, OBJECT_NAME, PARAM_JSON, SCHEDULER_INFO, STATUS, CREATED_BY, CREATED_DATE, JOB_RULE, callback) {
    var resdata = {};
    headers = objLogInfo["headers"] || "";
    try {
...
function updateJobsLog(mResCas, query, objLogInfo, status, start_time, end_time, error_msg, job_name, job_description, app_id, routing_key, callback, job_inc_id) {
    try {
...
function addThreadLogVariableParameter(mResCas, query, objLogInfo, schjtl_id, job_code, thread_id, status, start_time, end_time, error_msg, APP_ID, ROUTING_KEY, result, callback) {
    try {
        var serviceModel = reqFXDBInstance.DBInstanceSession['SERVICE_MODEL'];
        if (objLogInfo && objLogInfo.doDBOperations) {
            ...
            if (query.startsWith("update")) {
                ...
                objsolrtdata.SCHJTL_ID = schjtl_id;
                ...
                objsolrtdata.JOB_INSTANCE_ID = schjtl_id;
                if (serviceModel.AUDIT_ARCHIVAL_MODEL == 'SOLR') {
                    solrinsertupdate();
                } else {
                    callback()
...
//Query
var QUERY_INSERT_JOB = "insert into SCH_JOBS(APP_ID,ROUTING_KEY,JOB_NAME,JOB_DESCRIPTION,TEMPLATE_NAME,JOB_TYPE,CALLING_METHOD,JOB_MODE,OBJECT_NAME,PARAM_JSON,SCHEDULER_INFO,STATUS,CREATED_BY,CREATED_DATE) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)";
var QUERY_DELETE_JOB_TEMPLATE = "delete from SCH_JOBS where JOB_NAME = %s and app_id = %s";

function createJobWithParam(pHeaders, params, objLogInfo, callback) {
    try {
                    ...
                    async.series([
                        ...
                        function (asynccallback) {
                            ...
                            deleteJob(mDevCas, QUERY_DELETE_JOB_TEMPLATE, objLogInfo, job_name, app_id, function (resDelObj) {
                                createJob(mDevCas, QUERY_INSERT_JOB, objLogInfo, app_id, routing_key, job_name, job_description, template_name, job_type, calling_method, job_mode, object_name, JSON.stringify(param_json), JSON.stringify(scheduler_info), "CREATED", user_id, new Date(), "",
...
function StopJobCommon(params, StopJobCommonCB) {
    try {
        ...
        reqFXDBInstance.GetFXDBConnection(mHeaders, cas_type, objLogInfo, function (mDevCas) {
            ...
            schedulerUtil = require('../util/schedulerUtil');
            resobj.STATUS = schedulerUtil.StopCronJob(mDevCas, objLogInfo, job_name, app_id);
...
function CheckAndGetRedisKey(redisInstance, pJob_info, CheckAndGetRedisKeyCB) {
    try {
        ...
        if (sessionIDRedisKey) {
            ...
            redisInstance.get(sessionIDRedisKey, function (error, redisKeyValue) {
                if (error) {
                    reqInstanceHelper.PrintError(ServiceName, null, 'ERR-JOBHELPER-0002', 'Error While Getting Redis Key Value...', error);
