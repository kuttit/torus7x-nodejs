/*
    @Description        : Helper file for BindVersionAttachments API
*/

// Require dependencies
var reqMoment = require('moment');
var reqLinq = require('node-linq').LINQ;
var reqInstanceHelper = require('../../../../../torus-references/common/InstanceHelper');
var reqCommon = require('./Common');
var reqDBInstance = require('../../../../../torus-references/instance/DBInstance');
var reqTranDBInstance = require('../../../../../torus-references/instance/TranDBInstance');
var objLogInfo = null;
var serviceName = 'BindVersionAttachmentsHelper';
var mSession = null;

//This will call when unexpected close or finish
function finishApiCall() {
    if (mSession) {
        reqTranDBInstance.CallRollback(mSession);
    }
}

// To bind version attachments
function bindVersionAttachments(pParams, pHeaders, pObjLogInfo, callback) {
    var objLstAttDetails = [];
    objLogInfo = pObjLogInfo;
    try {
        var intTRNAId = parseInt(pParams.TRNA_ID);
        var objTRNA = [];
        reqTranDBInstance.GetTranDBConn(pHeaders, false, function(pSession) {
            try {
                var objCond = new Object();
                objCond['TRNA_ID'] = intTRNAId;
                reqTranDBInstance.GetTableFromTranDB(pSession, 'TRN_ATTACHMENTS', objCond, objLogInfo, function(pResultFromDb) {
                    try {
                        pResultFromDb = reqInstanceHelper.ArrKeyToUpperCase(pResultFromDb, objLogInfo);
                        if (pResultFromDb && pResultFromDb.length) {
                            prepareAttachmentDetails(pSession, pResultFromDb[0], pParams, pHeaders, "N", "", "N", "N", function(error, objAttDetail, info) {
                                try {
                                    if (error) {
                                        return callback(prepareErrorData(error, 'ERR-RES-71920', 'Error in prepareAttachmentDetails callback'));
                                    } else if (info) {
                                        return callback(null, null, info);
                                    } else {
                                        objLstAttDetails.push(objAttDetail);
                                        if (pResultFromDb[0].GROUP_ID) {
                                            var objCond = new Object();
                                            objCond['GROUP_ID'] = pResultFromDb[0].GROUP_ID;
                                            objCond['IS_CURRENT'] = 'N';
                                            reqTranDBInstance.GetTableFromTranDB(pSession, 'TRN_ATTACHMENTS', objCond, objLogInfo, function(objTRNA) {
                                                try {
                                                    objTRNA = reqInstanceHelper.ArrKeyToUpperCase(objTRNA, objLogInfo);
                                                    if (objTRNA && objTRNA.length) {
                                                        var count = 0;
                                                        doIteration(objTRNA[count]);

                                                        function doIteration(item) {
                                                            try {
                                                                prepareAttachmentDetails(pSession, item, pParams, pHeaders, "N", "", "N", "N", function(error, objAttDetail, info) {
                                                                    try {
                                                                        if (error) {
                                                                            return callback(prepareErrorData(error, 'ERR-RES-71919', 'Error in prepareAttachmentDetails callback'));
                                                                        } else if (info) {
                                                                            return callback(null, null, info);
                                                                        } else {
                                                                            if (objLstAttDetails[0].AttId != objAttDetail.AttId) {
                                                                                objLstAttDetails.push(objAttDetail);
                                                                            }
                                                                            if ((objTRNA.length - 1) == count) {
                                                                                var objLstAttDetailsSorted = new reqLinq(objLstAttDetails)
                                                                                    .OrderBy(function(t) {
                                                                                        return t.VersionNo;
                                                                                    })
                                                                                    .ToArray();
                                                                                return callback(null, objLstAttDetailsSorted);
                                                                            } else {
                                                                                count++;
                                                                                doIteration(objTRNA[count]);
                                                                            }
                                                                        }
                                                                    } catch (error) {
                                                                        return callback(prepareErrorData(error, 'ERR-RES-71905', 'Error in bindVersionAttachments function'));
                                                                    }
                                                                });
                                                            } catch (error) {
                                                                return callback(prepareErrorData(error, 'ERR-RES-71906', 'Error in bindVersionAttachments function'));
                                                            }
                                                        }
                                                    } else {
                                                        return callback(null, objLstAttDetails);
                                                    }
                                                } catch (error) {
                                                    return callback(prepareErrorData(error, 'ERR-RES-71907', 'Error in bindVersionAttachments function'));
                                                }
                                            });
                                        } else {
                                            return callback(null, null, 'No groupid found');
                                        }
                                    }
                                } catch (error) {
                                    return callback(prepareErrorData(error, 'ERR-RES-71908', 'Error in bindVersionAttachments function'));
                                }
                            });
                        } else {
                            return callback(null, null, 'No Attachments found');
                        }
                    } catch (error) {
                        return callback(prepareErrorData(error, 'ERR-RES-71909', 'Error in bindVersionAttachments function'));
                    }
                });
            } catch (error) {
                return callback(prepareErrorData(error, 'ERR-RES-71910', 'Error in bindVersionAttachments function'));
            }
        });
    } catch (error) {
        return callback(prepareErrorData(error, 'ERR-RES-71911', 'Error in bindVersionAttachments function'));
    }
}

// To format values
function __FormatValues(Pattern, pUserid, pUsername, callback) {
    try {
        var DefValue = Pattern;
        if (DefValue) {
            var dtNow = Date.now();
            DefValue = DefValue.replace("$DATE1$", reqMoment(dtNow).format('DDMMYYYY')); //DateTime.Now.ToString("ddMMyyyy"))
            DefValue = DefValue.replace("$DATE2$", reqMoment(dtNow).format('MMDDYYYY')); //DateTime.Now.ToString("MMddyyyy"))
            DefValue = DefValue.replace("$DATE3$", reqMoment(dtNow).format('DDMMYY')); //DateTime.Now.ToString("ddMMyy"))
            DefValue = DefValue.replace("$DATE4$", reqMoment(dtNow).format('MMDDYY')); //DateTime.Now.ToString("MMddyy"))
            DefValue = DefValue.replace("$TIME1$", reqMoment(dtNow).format('HHmm')); //DateTime.Now.ToString("hhmm"))
            DefValue = DefValue.replace("$TIME2$", reqMoment(dtNow).format('HHmmss')); //DateTime.Now.ToString("hhmmss"))
            DefValue = DefValue.replace("$UID$", pUserid.toString());
            DefValue = DefValue.replace("$USERNAME$", pUsername);
            DefValue = DefValue.replace("$TICKS$", ((dtNow * 10000) + 621355968000000000).toString());
            var d = new Date();
            d.setDate(d.getDate() - 1);
            DefValue = DefValue.replace("$TICKSTODAY$", ((dtNow * 10000) + 621355968000000000) - (d * 10000) + 621355968000000000).toString();
        }
        return callback(DefValue);
    } catch (error) {
        reqInstanceHelper.PrintError(serviceName, objLogInfo, 'ERR-RES-71912', 'Error in __FormatValues function', error);
        return callback(error);
    }
}

// To prepare attachment details
function prepareAttachmentDetails(pSession, objTRNA, pParams, pHeaders, strNeedAnnotation, strStampAnnotations, strNeedPaging, strNeedActionFilter, callback) {
    try {
        var isWatermarkInfo = false;
        var isCodeDescInfo = false;
        var isListActions = false;
        var isListAnn = false;

        var strWaterMarkText = "";
        var strFontStyle = "";
        var strFontSize = "";
        var strTransparency = "";
        var RelPath = objTRNA.RELATIVE_PATH;
        //reqDBInstance.GetCassandraConn(pHeaders, 'dep_cas', function (CassandraIns) {
        reqDBInstance.GetFXDBConnection(pHeaders, 'dep_cas', objLogInfo, function(pClient) {
            if (objTRNA.WATERMARK_CODE) {
                try {
                    const wmstmt = "Select * from watermark_info where app_id = ? and wmt_code = ?";
                    var cond = new Object();
                    cond.APP_ID = pParams.APP_ID;
                    cond.WMT_CODE = objTRNA.WATERMARK_CODE;
                    //pClient.execute(wmstmt, [pParams.APP_ID, objTRNA.WATERMARK_CODE], { prepare: true }, function (error, result) {
                    reqDBInstance.GetTableFromFXDB(pClient, 'WATERMARK_INFO', [], cond, objLogInfo, function(error, result) {
                        if (error) {
                            reqInstanceHelper.PrintError(serviceName, objLogInfo, 'ERR-RES-71913', 'Error in prepareAttachmentDetails function', error);
                        } else {
                            var wmtemplates = result.rows;
                            for (var i = 0; i < wmtemplates.length; i++) {
                                var rwWM = wmtemplates[i];
                                if (rwWM.template_json) {
                                    var jsn = JSON.parse(rwWM.template_json);
                                    strWaterMarkText = jsn.WATERMARK_TEXT.toString();
                                    __FormatValues(strWaterMarkText, pParams.U_ID, pParams.USER_NAME, function(val) {
                                        strWaterMarkText = val;
                                    });
                                    strFontStyle = jsn.FONT.toString();
                                    strTransparency = jsn.TRANSPARENCY.toString();
                                    if (parseInt(strTransparency) > 100) {
                                        strTransparency = "20";
                                    }
                                }
                            }
                        }
                        isWatermarkInfo = true;
                        successCall();
                    });
                } catch (error) {
                    reqInstanceHelper.PrintError(serviceName, objLogInfo, 'ERR-RES-71914', 'Error in prepareAttachmentDetails function', error);
                    return callback(error);
                }
            } else {
                isWatermarkInfo = true;
            }

            // Get Attachment Thumb nail icon data from attachment types
            var strATData = "data:image/x-icon;base64,"

            if (objTRNA.AT_CODE && objTRNA.AT_CODE != 'UNKNOWN') {
                try {
                    const AttTypesStmt = "Select * from code_desc_info where app_id = ? and cd_code = ?";
                    var cond = new Object();
                    cond.APP_ID = pParams.APP_ID;
                    cond.CD_CODE = 'ATTACHMENT_TYPES';
                    //pClient.execute(AttTypesStmt, [pParams.APP_ID, 'ATTACHMENT_TYPES'], { prepare: true }, function (errOR, lstAttTypes) {
                    reqDBInstance.GetTableFromFXDB(pClient, 'CODE_DESC_INFO', [], cond, objLogInfo, function(error, result) {
                        if (error) {
                            reqInstanceHelper.PrintError(serviceName, objLogInfo, 'ERR-RES-71915', 'Error in prepareAttachmentDetails function', error);
                        } else {
                            var lstAttTypes = result.rows;
                            if (lstAttTypes.length) {
                                var lstjsn = JSON.parse(lstAttTypes[0].value_json);
                                for (var i = 0; i < lstjsn.length; i++) {
                                    var jsn = lstjsn[i];
                                    if (jsn.Item['AT_CODE'] == objTRNA.AT_CODE) {
                                        strATData = "data:image/x-icon;base64," + jsn.Item["AT_ICON_DATA"];
                                        break;
                                    }
                                }
                            } else {
                                // return callback(null, null, 'No Attachment Types found');
                            }
                        }
                        isCodeDescInfo = true;
                        successCall();
                    });
                } catch (error) {
                    reqInstanceHelper.PrintError(serviceName, objLogInfo, 'ERR-RES-71916', 'Error in prepareAttachmentDetails function', error);
                    return callback(error);
                }
            } else {
                isCodeDescInfo = true;
            }

            //Get Transaction Allocations
            var lstActions = [];
            if (strNeedActionFilter == 'Y') {
                try {
                    // var objTableSelectDetails = new Object();
                    // var arrConditions = [];
                    // arrConditions.push({ column: 'ITEM_ID', value: objTRNA.TRNA_ID });
                    // arrConditions.push({ column: 'ITEM_TYPE', value: "ATMT" });
                    // arrConditions.push({ column: 'ALLOCATED_APPU_ID', value: pParams.APPU_ID });
                    // arrConditions.push({ column: 'ALLOCATED_APPUSER_NAME', value: pParams.USER_NAME });
                    // objTableSelectDetails.tableName = 'TRANSACTION_ALLOCATIONS';
                    // objTableSelectDetails.conditions = arrConditions;
                    var objCond = new Object();
                    objCond['ITEM_ID'] = objTRNA.TRNA_ID;
                    objCond['ITEM_TYPE'] = 'ATMT';
                    objCond['ALLOCATED_APPU_ID'] = pParams.APPU_ID;
                    objCond['ALLOCATED_APPUSER_NAME'] = pParams.USER_NAME;
                    reqTranDBInstance.GetTableFromTranDB(pSession, 'TRANSACTION_ALLOCATIONS', objCond, objLogInfo, function(objTrnAlloc) {
                        objTrnAlloc = reqInstanceHelper.ArrKeyToUpperCase(objTrnAlloc, objLogInfo);
                        if (objTrnAlloc && objTrnAlloc.length) {
                            var intTRNALLOCId = objTrnAlloc[0].TA_ID;
                            // var objTableSelectDetails = new Object();
                            // var arrConditions = [];
                            // arrConditions.push({ column: 'TA_ID', value: intTRNALLOCId });
                            // objTableSelectDetails.tableName = 'TRANSACTION_ALLOCATION_ACTIONS';
                            // objTableSelectDetails.conditions = arrConditions;
                            var objCond = new Object();
                            objCond['TA_ID'] = intTRNALLOCId;
                            reqTranDBInstance.GetTableFromTranDB(pSession, 'TRANSACTION_ALLOCATION_ACTIONS', objCond, objLogInfo, function(objTrnAllocActions) {
                                objTrnAllocActions = reqInstanceHelper.ArrKeyToUpperCase(objTrnAllocActions, objLogInfo);
                                if (objTrnAllocActions && objTrnAllocActions.length) {
                                    lstActions = new reqLinq(objTrnAllocActions)
                                        .Select(function(item) {
                                            return item.ACTION_CODE;
                                        })
                                        .ToArray();
                                }
                                isListActions = true;
                                successCall();
                            });
                        } else {
                            isListActions = true;
                            successCall();
                        }
                    });
                } catch (error) {
                    reqInstanceHelper.PrintError(serviceName, objLogInfo, 'ERR-RES-71917', 'Error in prepareAttachmentDetails function', error);
                    return callback(error);
                }
            } else {
                isListActions = true;
            }

            var ImageColor = "";
            var Dttadif_id = 0;
            var ImageFormat = "";
            var Dttad_id = objTRNA.DTTAD_ID;

            //Get DTTAD Image Format Details
            var strDTTADImageDetails = '';
            if (strDTTADImageDetails) {
                var jsnDTTADImg = JSON.parse(strDTTADImageDetails);
                ImageColor = jsnDTTADImg.Item["IMAGE_COLOR"];
                ImageFormat = jsnDTTADImg.Item["IMAGE_FORMAT"];
                Dttadif_id = jsnDTTADImg.Item["DTTADIF_ID"];
            }


            //Get AnnotationDetails
            var lstAnn = [];
            try {
                // var objTableSelectDetails = new Object();
                // var arrConditions = [];
                // arrConditions.push({ column: 'TRN_ID', value: objTRNA.TRNA_ID });
                // objTableSelectDetails.tableName = 'TRN_ANNOTATIONS';
                // objTableSelectDetails.conditions = arrConditions;
                var objCond = new Object();
                objCond['TRN_ID'] = objTRNA.TRNA_ID;
                reqTranDBInstance.GetTableFromTranDB(pSession, 'TRN_ANNOTATIONS', objCond, objLogInfo, function(lstTrnAnnt) {
                    lstTrnAnnt = reqInstanceHelper.ArrKeyToUpperCase(lstTrnAnnt, objLogInfo);
                    for (var i = 0; i < lstTrnAnnt.length; i++) {
                        var drAnn = lstTrnAnnt[i];
                        var annotation = new reqCommon.Annotation();
                        annotation.AnnotationId = drAnn.ANNOTATION_ID;
                        annotation.AnnotationType = drAnn.ANNOTATION_TYPE;
                        annotation.AnnotationText = drAnn.ANNOTATION_TEXT;
                        annotation.AnnotationData = drAnn.ANNOTATION_DATA;
                        lstAnn.push(annotation);
                    }
                    isListAnn = true;
                    successCall();
                });

                if (strStampAnnotations != '') {
                    var jsnAnn = JSON.parse(strStampAnnotations);
                    if (jsnAnn) {
                        strStampAnnotations = '';
                        for (var i = 0; i < jsnAnn.length; i++) {
                            var objJsn = jsnAnn[i];
                            if (objJsn.ANNOTATION_TYPE.toUpperCase() == 'STAMP') {
                                var strStmp = '';
                                strStmp = "<div>" + objJsn.ANNOTATION_TEXT + "</div>";
                                strStampAnnotations = strStampAnnotations + strStmp;
                            }
                        }
                    }
                }
            } catch (error) {
                reqInstanceHelper.PrintError(serviceName, objLogInfo, 'ERR-RES-71918', 'Error in prepareAttachmentDetails function', error);
                return callback(error);
            }

            var strChkOut = '';
            if (objTRNA.CHECKED_OUT_DATE) {
                strChkOut = objTRNA.CHECKED_OUT_DATE;
            }

            var strVersionNo = objTRNA.VERSIONING;
            // if (!strVersionNo) {
            //     strVersionNo = '1';
            // }
            function successCall() {
                if (isWatermarkInfo && isCodeDescInfo && isListActions && isListAnn) {
                    var attachmentDetails = new reqCommon.AttachmentDetails();
                    attachmentDetails.AttId = objTRNA.TRNA_ID;
                    attachmentDetails.ATCode = objTRNA.AT_CODE;
                    attachmentDetails.FilePath = RelPath;
                    attachmentDetails.ViewerType = pParams.VIEWER_TYPE;
                    attachmentDetails.ATData = strATData;
                    attachmentDetails.WaterMarkText = strWaterMarkText;
                    attachmentDetails.font = strFontStyle;
                    attachmentDetails.fontsize = strFontSize;
                    attachmentDetails.Transparency = strTransparency;
                    attachmentDetails.Actions = lstActions;
                    attachmentDetails.LoadPageByPage = strNeedPaging;
                    attachmentDetails.ImageColor = ImageColor;
                    attachmentDetails.ImageFormat = ImageFormat;
                    attachmentDetails.Dttad_id = Dttad_id;
                    attachmentDetails.Dttadif_id = Dttadif_id;
                    attachmentDetails.Annotations = lstAnn;
                    attachmentDetails.NeedAnnotation = strNeedAnnotation;
                    attachmentDetails.CBOStampAnnotations = strStampAnnotations;
                    attachmentDetails.RS_STORAGE_TYPE = pParams.RS_STORAGE_TYPE;
                    attachmentDetails.RS_DB_INFO = pParams.RS_DB_INFO;
                    attachmentDetails.Userid = pParams.U_ID;
                    attachmentDetails.GroupId = objTRNA.GROUP_ID;
                    attachmentDetails.VersionNo = 'V' + strVersionNo;
                    attachmentDetails.CheckOutBy = objTRNA.CHECKED_OUT_BY;
                    attachmentDetails.CheckOutDate = strChkOut;
                    attachmentDetails.IsCurrent = objTRNA.IS_CURRENT;
                    attachmentDetails.CheckOutByName = objTRNA.CHECKED_OUT_BY_NAME;
                    attachmentDetails.OriginalFileName = objTRNA.ORIGINAL_FILE_NAME;
                    return callback(null, attachmentDetails);
                }
            }
        });
    } catch (error) {
        reqInstanceHelper.PrintError(serviceName, objLogInfo, 'ERR-RES-71901', 'Error in prepareAttachmentDetails function', error);
        return callback(error);
    }
}

// To prepare error data
function prepareErrorData(error, errorCode, errorMessage) {
    var errJson = {
        ERROR: error,
        ERROR_CODE: errorCode,
        ERROR_MESSAGE: errorMessage
    }
    return errJson;
}

module.exports = {
        BindVersionAttachments: bindVersionAttachments,
        FinishApiCall: finishApiCall
    }
    /******** End of File *******/