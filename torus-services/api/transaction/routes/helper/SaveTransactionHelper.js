...
function getDTTDescription(pClient, pAppID, pDTTCode, objLogInfo, callback) {
    try {
        var strDTTDesc = '';
        if (pDTTCode) {
            var cond = new Object();
                ...
                if (dataBindings) {
                    for (var k = 0; k < dataBindings.length; k++) {
                        ...
                        if (data.type && data.type == 'date') {
                            itm[data.field] = reqDateFormatter.GetDateAt12AM(pHeaders, objLogInfo, itm[data.field]);
                        }
                    }
                            ...
                            try {
                                ...
                                reqDBInstance.GetTableFromFXDB(pClient, 'DT_INFO', [], cond, objLogInfo, function (error, result) {
                                    try {
                                        ...
                                        reqCommon.DoFilterRecursiveArr(objDTTRelation, strDTTCode, 'DTT_CODE', 'CHILD_DTT_RELEATIONS', objLogInfo, function (objDTTR) {
                                            try {
                                        ...
                                        function arrFromClientLoop(objItemSet) {
                                            j++;
                                            ...
                                            if (arrItems.length) {
                                                for (var k = 0; k < arrItems.length; k++) {
                                                    ...
                                                    if (!objItem.Key_Value || objItem.Key_Value == 0) { // Insert Case (New Record)
                                                        ...
                                                        objItem.MODIFIED_BY = strUserid;
                                                        objItem.MODIFIED_BY_NAME = strLoginName;
                                                        objItem.PRCT_ID = objLogInfo.PROCESS_INFO.PRCT_ID;
                                                        kCount++;
                                                        if (kCount >= arrItems.length) {
                                                            if (j < arrFromClient.length) {
