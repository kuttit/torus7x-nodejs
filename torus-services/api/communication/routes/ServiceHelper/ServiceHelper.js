...
function GetTRNDetails(pTranDB, pOrm, pDepCas, pTRNQuery, pAppId, pDTCode, pDTTCode, pWftpaId, pUid, pPrctId, pLogInfo, pTemplateInfo, psessionInfo, pCallback) {
    try {
        ...
        if (pTRNQuery != undefined && pTRNQuery != '') {
            ...
            if (static_data) {
                staticDataResult.rows = [static_data];
            }
            else {
                _PrintInfo('', pLogInfo);
                // This is to send the Email and SMS Notifaction without TRN Query or Any Static Data
                var hardCodedData = { Info: 'Case Without TRN Query or Any Static Data' };
                staticDataResult.rows = [hardCodedData];
            }
...
function GetEmailIDFromFXDB(pCltCas, pData, pContactInfos, pAppID, pClientID, pType, pLogInfo, pCallback) {
    try {
        if (pContactInfos[0]['COLUMN_NAME'].split('|')[0] === 'USERS') {
        ...
        } else if (pContactInfos[0]['COLUMN_NAME'].split('|')[0] === 'ROLES') {
