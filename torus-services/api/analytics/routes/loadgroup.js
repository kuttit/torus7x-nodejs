...
// Host the login api
router.post('/loadgroup', function(appReq, appResp) {
    reqLogInfo.AssignLogInfoDetail(appReq, function (objLogInfo, objSessionInfo) {
        
    reqLogWriter.Eventinsert(objLogInfo);
    objLogInfo.PROCESS = 'loadgroup-Analytics';
    objLogInfo.ACTION = 'loadgroup';
    var strHeader = {};
    ...
    try {
        ...
        var Select_Group="select distinct group_name from program_group where project_id="+Project_Id+"";
        reqAnalyticInstance.GetTranDBConn(strHeader,false,function callbackGetTranDB(pSession){
         try {
            reqAnalyticInstance.ExecuteSQLQuery(pSession, Select_Group,objLogInfo, function callbackTransactionSetUpdate(pResult, pError) {
             try {
                if(pError){
                    _SendResponse({},'Errcode','Error In Getting Group',pError,null );
                }
                else{
                    var result=pResult
