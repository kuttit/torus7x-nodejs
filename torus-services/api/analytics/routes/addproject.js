...
// Host the login api
router.post('/addproject', function (appReq, appResp) {

    reqLogInfo.AssignLogInfoDetail(appReq, function (objLogInfo, objSessionInfo) {
        reqLogWriter.Eventinsert(objLogInfo);
        objLogInfo.PROCESS = 'AddProject-Analytics';
        objLogInfo.ACTION = 'AddProject';
        var strHeader = {};
        ...
        try {
            ...
            var params = appReq.body;
            var Projectname = appReq.body[0].Projectname;
            var Projectdescription = appReq.body[0].Projectdescription;
            var userid = appReq.body[0].userid;
            var appid = appReq.body[0].appid;
            var systemid = appReq.body[0].systemid;
            var clientid = appReq.body[0].clientid;
            var appuid = appReq.body[0].appuid;
            var clustercode = appReq.body[0].clustercode;



            reqTranDBInstance.GetTranDBConn(strHeader, false, function callbackGetTranDB(pSession) {
                reqTranDBInstance.InsertTranDBWithAudit(pSession, 'projects', [{
