...
// Host the login api
router.
post('/hueuserdelete', function(appReq, appResp) {

    reqLogInfo.AssignLogInfoDetail(appReq, function (objLogInfo, objSessionInfo) {
        reqLogWriter.Eventinsert(objLogInfo);
        objLogInfo.PROCESS = 'hueuserdelete-Analytics';
        objLogInfo.ACTION = 'hueuserdelete';
        ...
        try {
            ...
            var id = params.PARAMS.toLowerCase();
            console.log(id);
            
            reqTranDBInstance.GetTranDBConn(strHeader,false,function callbackGetTranDB(pSession){
            reqTranDBInstance.UpdateTranDBWithAudit(pSession, 'auth_user', {
                is_active:false
            }, {
                username:id
