...
// Host the login api
router.post('/sparkkeys', function(appReq, appResp) {

    reqLogInfo.AssignLogInfoDetail(appReq, function (objLogInfo, objSessionInfo) {
        reqLogWriter.Eventinsert(objLogInfo);
    
        objLogInfo.PROCESS = 'sparkkeys-Analytics';
        objLogInfo.ACTION = 'sparkkeys';
            ...
            { _SendResponse({},'Errcode','Error while update Programs Table',err,null );
                ...
                sparkkeys=[];
                 for(var i = 0, len = keys.length; i < len; i++) {
                     if(keys[i].slice(0, 6)=="spark_"){
             
              sparkkeys.push(keys[i]);
                     }
            }
            _SendResponse(sparkkeys,'','',null,null );
            }
