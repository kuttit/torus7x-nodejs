...
// Host the login api
router.post('/editanalyticalproject', function (appReq, appResp) {

    reqLogInfo.AssignLogInfoDetail(appReq, function (objLogInfo, objSessionInfo) {
        reqLogWriter.Eventinsert(objLogInfo);
        objLogInfo.PROCESS = 'editanalyticalproject-Analytics';
        objLogInfo.ACTION = 'editanalyticalproject';
        var strHeader = {};
        ...
        try {
            ...
            var id=appReq.body[0].projectid;


            reqTranDBInstance.GetTranDBConn(strHeader, false, function callbackGetTranDB(pSession) {
                reqTranDBInstance.UpdateTranDBWithAudit(pSession, 'projects', {
                    "project_name": Projectname,
                    "project_description": Projectdescription,
                    "user_id": userid
                },{"id":id} ,objLogInfo, function callbackTransactionSetUpdate(pResult, pError) {
                    if (pError){
